...
target_dim <- c("pw_identity")  # Decode dimensions to analyze
crossval_method <- c("one_rotation_out")  # Cross-validation method(s)

# Optional pairwise mode -------------------------------------------
# Options: "all", "hetero", "animate_only", "inanimate_only"
pairwise_mode <- c("all", "hetero", "animate_only", "inanimate_only") # If not supplied, leave as NULL
...
# Build combinations conditionally --------------------------------
if (!is.null(pairwise_mode) && length(pairwise_mode) > 0) {
...
combinations <- combinations %>%
  mutate(method = case_when(
    type %in% c('entory', 'luminance', 'size_2class', '3class') ~ 'one_block_out', 
...
filenames <- if ("pairwise_mode" %in% names(combinations)) {
  paste(combinations$type, combinations$method, combinations$pairwise_mode, sep = "_")
} else {
  paste(combinations$type, combinations$method, sep = "_")
}
