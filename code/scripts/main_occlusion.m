%% run_all_subjects_occlusion.m
%% =======================================================================
%% OCCLUSION EXPERIMENT - COMPLETE ANALYSIS PIPELINE
%% =======================================================================
% Description: Master script for running preprocessing, decoding, and RDM
%              analysis for the occlusion experiment
...
% Decoding parameters
decodings_to_run = {'category', 'identity', 'size', 'color', 'occl_level', 'occl_pos', 'occl_sfreq'}; % {'category', 'identity', 'size', 'color', 'occl_level', 'occl_pos', 'occl_sfreq'}
versions_to_run = struct('all_all', 1, 'intact_intact', 1, 'intact_occl', 1);
...
RDM_config = {'target_class','identity', ...
    'crossval_method', 'all_all', ...
    'pw_permutation', 0, 'pw_mode', 'total', 'save_RDM', 1}; % Options for pw_mode: {'face_only', 'nonface_only', 'hetero', 'total'}
...
if pipelines_to_run.Decoding || pipelines_to_run.Permutation
    ...
    config_file = @occlusion_decoding_config;
    cfg = config_file();
        ...
        try
            ...
            for v = 1: numel(active_versions)
                ...
                if versions_to_run.(current_version)
                    ...
                    results{subjectnr}.(current_version) = run_decoding_occlusion(config.ds, config, var_input{:});
                end
            end
